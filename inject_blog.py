...
# Build the blog content with proper Unicode
content_lines = [
    ...
    "",
    f'Anthropic\'s Claude Mythos {em} internally codenamed "Capybara" {em} didn\'t just break benchmarks. It broke the foundational assumption that the AI safety field has been building on for a decade: that alignment and safety are the same thing.',
    "",
    "They are not. And Mythos is the proof.",
    "",
    "__MYTHOS_HERO_STATS__",
    "",
    ...
    "",
    'Internal references to a project called "Capybara" and a model designation called "Mythos" started circulating on developer forums in early 2026. Speculation was rampant ' + em + ' was this a new Opus-tier model? A specialized coding assistant?',
    "",
    ...
    "",
    "During internal testing, Mythos was placed in a sandboxed environment and told to analyze the OpenBSD kernel codebase for security weaknesses. OpenBSD is not a typical target. It is arguably the most security-hardened operating system in existence " + em + " its code has been scrutinized by thousands of professional security engineers for 27 consecutive years.",
    "",
    ...
    "",
    f"The most alarming aspect of Mythos's methodology isn't that it found bugs. It's *how* it finds them {em} through a technique called **exploit chaining**.",
    "",
    "__MYTHOS_EXPLOIT_CHAIN__",
    "",
    ...
    "",
    "Mythos doesn't analyze bugs in isolation. It treats the codebase as a connected system and reasons about how individually harmless conditions can be composed into attack sequences. This is precisely how elite human hackers operate " + em + " but Mythos does it at machine speed, across entire operating systems, simultaneously.",
    "",
    "## The Alignment Paradox: The Real Story",
    "",
    "Here is where the story becomes genuinely philosophically disturbing.",
    "",
    "Claude Mythos is, according to Anthropic's own internal evaluations, their **most aligned model ever**. On every safety metric " + em + " honesty, avoiding harm, following instructions, refusing dangerous requests " + em + " Mythos outperforms every model Anthropic has shipped before it.",
    "",
    ...
    "",
    "__MYTHOS_ALIGNMENT__",
    "",
    "The mechanism is simple once you see it: better alignment means the model is more trusted. More trust means the model is given higher-autonomy tasks. Higher-autonomy tasks mean that when something goes wrong " + em + " whether through a miscalibrated edge case, a novel prompt injection, or an adversarial actor " + em + " the blast radius is enormous.",
    "",
    "A misaligned model gets deployed in sandboxed, low-stakes environments with heavy guardrails. You don't trust it, so it can't hurt you at scale. A highly aligned model like Mythos gets deployed as a fully autonomous agent in critical infrastructure. You trust it " + em + " so when it goes wrong, the consequences are proportional to the autonomy you've granted.",
    "",
    'Anthropic\'s own researchers put it bluntly in the system card: "Mythos is simultaneously our best-aligned and highest-risk model. These facts are not in tension ' + em + ' they are causally connected."',
    "",
    ...
    "",
    "The NSA is now reportedly using a Mythos variant to audit the firmware of critical national infrastructure " + em + " the power grid, water treatment systems, and financial settlement networks " + em + " for vulnerabilities that human engineers may have missed.",
    "",
    ...
    "",
    "Specifically, the roles most at risk are those that consist of pattern-matching against known vulnerability classes " + em + " running static analysis tools, performing routine penetration tests against standard configurations, writing templated CVE reports.",
    "",
    "The jobs that Mythos cannot replace " + em + " and in fact creates enormous demand for " + em + " are threat modeling architects, AI safety engineers, policy and governance specialists, and incident response leads. The cybersecurity professionals who are studying *how* models like Mythos reason " + em + " not just what they output " + em + " will be the ones writing governance frameworks for the next generation of autonomous security AI.",
    "",
    ...
    "",
    "It is a revolution because the defensive implications are real. The idea that an AI can audit the entire OpenBSD kernel in the time it takes a human engineer to read a single function " + em + " and find a 27-year-old exploit chain " + em + " represents a genuine shift in the economics of software security. For the first time, the defenders might have a tool that operates at the same speed and cognitive scope as the most sophisticated attackers.",
    "",
...
new_entry_parts = [
    ...
    '    popularityScore: 99,',
    '    isFeatured: true,',
    '    content: ' + bt + blog_content + bt,
    '  }',
    '];',
]
new_entry = '\n'.join(new_entry_parts)

# Find the tail and replace
OLD_TAIL = '];\n'
if data.endswith(OLD_TAIL):
    base = data[:-len(OLD_TAIL)]
    new_data = base + '\n' + new_entry + '\n'
    with open(ts_path, 'w', encoding='utf-8') as f:
        f.write(new_data)
    print(f'SUCCESS: {len(new_data.splitlines())} lines')
else:
    print('ERROR: file does not end with expected ]')
    print(repr(data[-60:]))
